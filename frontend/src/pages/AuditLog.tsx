    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <Input
            placeholder="Search entity ID, user ID, action…"
            className="pl-9"
            value={search}
        ...
        <Select value={entityType} onValueChange={(v) => { setEntityType(v); setPage(0) }}>
          ...
          <SelectContent>
            ...
            <SelectItem value="pay_source">Pay Source</SelectItem>
          </SelectContent>
        </Select>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  ...
                  <th className="text-left px-4 py-3 font-medium text-gray-600">Fields Changed</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-600">IP</th>
                </tr>
              </thead>
              <tbody>
                {!filtered?.length ? (
                  <tr><td colSpan={7} className="text-center py-10 text-gray-400">No audit entries found</td></tr>
                ) : filtered.map((e) => (
                  <tr key={e.id} className="border-b border-gray-50 hover:bg-gray-50">
                    <td className="px-4 py-3 text-gray-500 whitespace-nowrap">{formatDateTime(e.created_at)}</td>
                    ...
                    <td className="px-4 py-3 font-mono text-xs text-gray-500 max-w-[120px] truncate">{e.entity_id}</td>
                    <td className="px-4 py-3 font-medium capitalize">{e.action.replace(/_/g, ' ')}</td>
                    <td className="px-4 py-3 font-mono text-xs text-gray-500 max-w-[120px] truncate">{e.user_id}</td>
